...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const { theme, setTheme, customColors, setCustomColors, language, setLanguage } = useTheme();
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-body-small text-muted-foreground">
                {t('settings.configure_claude_preferences')}
              </p>
            </div>
            <motion.div
              whileTap={{ scale: 0.97 }}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid grid-cols-8 w-full mb-6 h-auto p-1">
              ...
              <TabsTrigger value="permissions" className="py-2.5 px-3">{t('settings.permissions_tab')}</TabsTrigger>
              <TabsTrigger value="environment" className="py-2.5 px-3">{t('settings.environment_tab')}</TabsTrigger>
              <TabsTrigger value="advanced" className="py-2.5 px-3">{t('settings.advanced_tab')}</TabsTrigger>
              <TabsTrigger value="hooks" className="py-2.5 px-3">{t('settings.hooks_tab')}</TabsTrigger>
              <TabsTrigger value="commands" className="py-2.5 px-3">{t('settings.commands_tab')}</TabsTrigger>
              <TabsTrigger value="storage" className="py-2.5 px-3">{t('settings.storage_tab')}</TabsTrigger>
              <TabsTrigger value="proxy" className="py-2.5 px-3">{t('settings.proxy_tab')}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="general" className="space-y-6 mt-6">
              <Card className="p-6 space-y-6">
                <div>
                  <h3 className="text-heading-4 mb-4">{t('settings.general_settings')}</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    {theme === 'custom' && (
                      <div className="space-y-4 p-4 border rounded-lg bg-muted/20">
                        <h4 className="text-label">{t('settings.custom_theme_colors')}</h4>
                        
                        <div className="grid grid-cols-2 gap-4">
                          {/* Background Color */}
                          <div className="space-y-2">
                            <Label htmlFor="color-background" className="text-caption">{t('settings.background')}</Label>
                          ...
                          {/* Foreground Color */}
                          <div className="space-y-2">
                            <Label htmlFor="color-foreground" className="text-caption">{t('settings.foreground')}</Label>
                            <div className="flex gap-2">
                              <Input
                                id="color-foreground"
                                type="text"
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5 flex-1">
                        <Label htmlFor="coauthored">{t('settings.include_coauthored_by')}</Label>
                        <p className="text-caption text-muted-foreground">
                          {t('settings.include_coauthored_by_description')}
                        </p>
                      </div>
                      <Switch
                        id="coauthored"
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5 flex-1">
                        <Label htmlFor="verbose">{t('settings.verbose_output')}</Label>
                        <p className="text-caption text-muted-foreground">
                          {t('settings.verbose_output_description')}
                        </p>
                      </div>
                      <Switch
                        id="verbose"
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <Label htmlFor="cleanup">{t('settings.chat_transcript_retention')}</Label>
                          <p className="text-caption text-muted-foreground mt-1">
                            {t('settings.chat_transcript_retention_description')}
                          </p>
                        </div>
                        <Input
                          id="cleanup"
                    ...
                    <div className="space-y-3">
                      ...
                      {binaryPathChanged && (
                        <p className="text-caption text-amber-600 dark:text-amber-400 flex items-center gap-1">
                          ...
                          {t('settings.changes_will_be_applied')}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        <Label htmlFor="analytics-enabled">{t('settings.enable_analytics')}</Label>
                        <p className="text-caption text-muted-foreground">
                          {t('settings.enable_analytics_description')}
                        </p>
                      </div>
                      <Switch
                        ...
                        onCheckedChange={async (checked) => {
                          if (checked) {
                            ...
                            setToast({ message: t('settings.analytics_enabled'), type: "success" });
                          } else {
                            await analytics.disable();
                            setAnalyticsEnabled(false);
                            trackEvent.settingsChanged('analytics_enabled', false);
                            setToast({ message: t('settings.analytics_disabled'), type: "success" });
                          }
                        }}
                      />
                    </div>
                    ...
                    {analyticsEnabled && (
                      <div className="rounded-lg border border-border bg-muted/50 p-3">
                        <div className="flex gap-2">
                          ...
                          <div className="space-y-1">
                            <p className="text-xs font-medium text-foreground">{t('settings.your_privacy_is_protected')}</p>
                            <ul className="text-xs text-muted-foreground space-y-0.5">
                              <li>• {t('settings.privacy_info')}</li>
                              <li>• {t('settings.privacy_anonymous')}</li>
                              <li>• {t('settings.privacy_disable')}</li>
                            </ul>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        <Label htmlFor="tab-persistence">{t('settings.remember_open_tabs')}</Label>
                        <p className="text-caption text-muted-foreground">
                          {t('settings.remember_open_tabs_description')}
                        </p>
                      </div>
                      <Switch
                        id="tab-persistence"
                        ...
                        onCheckedChange={(checked) => {
                          ...
                          setToast({ 
                            message: checked 
                              ? t('settings.tab_persistence_enabled') 
                              : t('settings.tab_persistence_disabled'), 
                            type: "success" 
                          });
                        }}
                      />
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        <Label htmlFor="startup-intro">{t('settings.show_welcome_intro')}</Label>
                        <p className="text-caption text-muted-foreground">
                          {t('settings.show_welcome_intro_description')}
                        </p>
                      </div>
                      <Switch
                        id="startup-intro"
                        ...
                        onCheckedChange={async (checked) => {
                          ...
                          try {
                            ...
                            setToast({ 
                              message: checked 
                                ? t('settings.welcome_intro_enabled') 
                                : t('settings.welcome_intro_disabled'), 
                              type: 'success' 
                            });
                          } catch (e) {
                            setToast({ message: t('settings.failed_to_update_preference'), type: 'error' });
                          }
                        }}
                      />
                    </div>
            ...
            <TabsContent value="permissions" className="space-y-6">
              <Card className="p-6">
                <div className="space-y-6">
                  <div>
                    <h3 className="text-heading-4 mb-2">{t('settings.permission_rules')}</h3>
                    <p className="text-body-small text-muted-foreground mb-4">
                      {t('settings.permission_rules_description')}
                    </p>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <Label className="text-label text-green-500">{t('settings.allow_rules')}</Label>
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <Label className="text-label text-red-500">{t('settings.deny_rules')}</Label>
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  <div className="pt-2 space-y-2">
                    <p className="text-xs text-muted-foreground">
                      <strong>{t('settings.permission_examples')}</strong>
                    </p>
            ...
            <TabsContent value="environment" className="space-y-6">
              <Card className="p-6">
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-heading-4">{t('settings.environment_variables')}</h3>
                      <p className="text-sm text-muted-foreground mt-1">
                        {t('settings.environment_variables_description')}
                      </p>
                    </div>
                    <Button
                      variant="outline"
                  ...
                  <div className="space-y-3">
                    {envVars.length === 0 ? (
                      <p className="text-xs text-muted-foreground py-2">
                        {t('settings.no_environment_variables')}
                      </p>
                    ) : (
                      envVars.map((envVar) => (
                        <motion.div
            ...
            <TabsContent value="advanced" className="space-y-6">
              <Card className="p-6">
                <div className="space-y-6">
                  ...
                  {/* API Key Helper */}
                  <div className="space-y-2">
                    <Label htmlFor="apiKeyHelper">{t('settings.api_key_helper')}</Label>
                    <Input
                      id="apiKeyHelper"
            ...
            <TabsContent value="hooks" className="space-y-6">
              <Card className="p-6">
                <div className="space-y-4">
                  <div>
                    <h3 className="text-base font-semibold mb-2">{t('settings.user_hooks')}</h3>
                    <p className="text-body-small text-muted-foreground mb-4">
